...
class TweetIngest extends Serializable {
  ...
  private var currentRateLimit: RateLimit = Await.result(restClient.rateLimits(), 10.seconds).data.resources.search.values.toList(0)
  ...
  private def searchHandler(query: String, product: Product): Unit = {
    ...
    search onComplete {
      ...
      case Failure(err: TwitterException) => {
        ...
        currentRateLimit = Await.result(restClient.rateLimits(), 10.seconds).rate_limit
  ...
  private def searchTweet(query: String): Future[RatedData[StatusSearch]] = {
    if (currentRateLimit != null) {
      if (currentRateLimit.remaining < 5) {
        ...
        println("Hit rate limit, pausing for " + duration.toMinutes + " minutes")
        progress()
        // Get the pause time
  ...
  private def progress(): Unit = {
    println("TweetIngest: done " + requestCount + " of " + totalRequests + " remaining in rate limit " + currentRateLimit.remaining)
  }
}
